...
quat vec_to_quat(vec3_c v1) {
	return (quat){0, v1.x, v1.y, v1.z};
}


quat quat_mul(quat q1, quat q2) {
	vec3_c q1_i = {q1.i, q1.j, q1.k};
	vec3_c q2_i = {q2.i, q2.j, q2.k};

	float scalar = q1.w * q2.w - vec3_dot(q1_i, q2_i);

	vec3_c imaginary = vec3_add(
			vec3_add(
	...
	vec3_c test = {1, 1, 1};
	vec3_c test2 = {1, 0, 0};
	vec3_c c_test = vec3_cross(test, test2);
...
quat quat_rotate(float theta, vec3_c axis, vec3_c v) {
